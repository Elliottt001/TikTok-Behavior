...
class DataGenerator:
    ...
    def csv_gen(self, file_name, row_count, row_func, headers):
        """
        file_name: 文件名
        row_count: 行数
        ...
        with open(file_name, 'w', encoding="UTF-8") as file:
            for i in range(0, logs_count):
                ...
                data = {
                    ...
                    "duration": duration,
                    "time_stamp": self.fake.date_time_between(start_date=datetime.strptime(v["update_time"], "%Y/%m/%d"), end_date='now').strftime("%Y/%m/%d %H:%M:%S"),
